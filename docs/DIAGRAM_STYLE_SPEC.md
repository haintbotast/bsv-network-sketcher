...
- **Rebalance port/side (L1):** sức chứa mỗi side tính theo **minSpacing = max(`bundle_gap` + (`port_label_offset` − 1 ký tự) + `label_height`/2, `label_height` + `label_gap_y`/2)** để đủ chỗ cho **anchor gap + label clearance**.
